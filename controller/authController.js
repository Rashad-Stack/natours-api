...
// when user forget his/her password and we send email for reseating password
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const randomToken = await user.generateRandomResetToken();
  await user.save({ validateBeforeSave: false });

  // 3) Send it to user email
  const resetUrl = `${req.protocol}://${req.get(
    "host"
  )}/api/v1/users/reset-password/${randomToken}`;
...
// when user forget his/her password
...
// when user already logged in and wants to change his/her password
exports.updatePassword = catchAsync(async (req, res, next) => {
  // 1) Get user from collection
  const user = await User.findById(req.user.id).select("+password");
  // 2) If Posted current password correct
